...
public class Task12A extends AdventTask {
    ...
    public List<String> perform(List<String> input) {
        ...
        List<String> paths = discoverNeighborhoods("start", "");

        return List.of(paths.size() + "");
    }

    private List<String> discoverNeighborhoods(String node, String prefix) {
        if (node.equals("end")) {
        ...
        for (String neighborhood : neighborhoods.get(node)) {
            paths.addAll(discoverNeighborhoods(neighborhood, prefix));
        }
        return paths;
    }
}
